document.addEventListener("DOMContentLoaded", function () {
    ...
    if (!eventoId) {
        avaliacaoContainer.innerHTML =
            "<div class='alert alert-danger'>Evento não identificado.</div>";
        return;
    }

    if (!usuarioLogado) {
        avaliacaoContainer.innerHTML =
            "<div class='alert alert-warning'>Você precisa estar logado para avaliar este evento. <a href='../login/loginregistro.html'>Faça login aqui</a>.</div>";
        return;
    }
    ...
    async function carregarAvaliacoes() {
        try {
            const response = await fetch(
                `${API_URL}/avaliacoeseventos?idevento=${eventoId}`,
            );
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            ...
            lista.innerHTML =
                '<h3 class="mb-4">Avaliações do evento</h3>' +
                avaliacoes
                    .map(
                        (av) => `
                <div class="card mb-3">
        ...
        try {
            ...
            btn.onclick = async function () {
                try {
                    ...
                    if (notaSelecionada === 0 || comentario === "") {
                        alert(
                            "Por favor, selecione uma nota e escreva um comentário.",
                        );
                        return;
                    }
